...
export class CategoryCollectionStateStub implements ICategoryCollectionState {
  ...
  constructor(readonly allScripts: Script[] = [new ScriptStub(
    `${CategoryCollectionStateStub.name}-script`,
  )]) {
    ...
    this.collection = new CategoryCollectionStub()
      ...
      .withAction(new CategoryStub(
        `${CategoryCollectionStateStub.name}-category`,
